...
## 项目信息
- 项目名称：projectDemo
- 技术栈：Next.js 15（App Router）· TypeScript · Shadcn/UI · Tailwind CSS · Zustand · TanStack Query · Vercel AI SDK
- 当前阶段：初始化 / 文档评估
- 目录结构：
  ```
  src/
  ├── app/        # App Router 路由、页面、全局 layout/provider
  ├── features/   # 按业务模块拆分
  ├── entities/   # 跨模块复用的核心业务实体
  ├── shared/     # 通用 UI、API 客户端、hooks、工具
...
启动开发服务前：
1. 检查端口占用：`lsof -i :3000` 或 `netstat -ano | findstr 3000`
2. 若已占用，检查服务健康：`curl http://localhost:3000`
3. 若不健康，清理后重启
...
### 服务管理
明确谁负责启动服务，统一使用固定端口（3000），启动前先检查现有服务，禁止重复启动。

### 集成验证
各 Agent 完成后必须通过 typecheck，集成前运行 test，最终验证运行 build。

## 常见坑位
- Next.js App Router 中 Server Component 不能使用 `useState`/`useEffect`，需标记 `'use client'`
- Shadcn/UI 组件通过 `pnpm dlx shadcn@latest add <component>` 安装，不要手写
- 环境变量客户端可用必须加 `NEXT_PUBLIC_` 前缀
- `src/generated/` 由脚本生成，修改 schema 后重新生成
